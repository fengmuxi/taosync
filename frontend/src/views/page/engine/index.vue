		...
		<div v-else class="card-box">
			<div class="card-item" v-for="item in alistList">
				<div class="card-item-top">
					<el-image src="/alist.svg" fit="contain" class="card-item-logo"></el-image>
				...
				<div class="card-item-bottom">
					...
					<el-button size="small" type="info" @click="testConnectionOnItem(item)">测试连接</el-button>
			...
			<div class="card-item card-add" @click="addShow" v-if="!getLoading">
				<template v-if="alistList.length == 0">
					暂无引擎，请<span class="highlight-text">新增</span>
				</template>
				<span v-else>新增</span>
			</div>
			...
			<span slot="footer" class="dialog-footer">
				...
				<el-button type="info" @click="testConnection" :loading="testLoading">测试连接</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			testConnection() {
				// 验证表单中的必填字段
				this.$refs.addRule.validateField(['url', 'token'], (valid) => {
					if (!valid) {
